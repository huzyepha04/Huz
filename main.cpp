...
public:
	...
	void writeToFile(ofstream& outFile) {
		outFile << "BOGIE ID: " << Bogie_ID << endl;
		outFile << "Adults:" << endl;
		for (int k = 0; k < y; k++) {
			outFile << "Name: " << dynamic_cast<Adult*>(Adults[k])->getname() << ", Age: "
				<< dynamic_cast<Adult*>(Adults[k])->getage() << ", Gender: "
				<< dynamic_cast<Adult*>(Adults[k])->getgender() << ", Occupation: "
				<< dynamic_cast<Adult*>(Adults[k])->getOccupation() << ", Qualification: "
				<< dynamic_cast<Adult*>(Adults[k])->getQualification() << ", NIC: "
				<< dynamic_cast<Adult*>(Adults[k])->getNIC() << endl;
		}
		outFile << "Kids:" << endl;
		for (int k = 0; k < z; k++) {
			outFile << "Name: " << dynamic_cast<kid*>(kids[k])->getname() << ", Age: "
				<< dynamic_cast<kid*>(kids[k])->getage() << ", Gender: "
				<< dynamic_cast<kid*>(kids[k])->getgender() << ", B-Form Number: "
				<< dynamic_cast<kid*>(kids[k])->getBform() << endl;
		}
		outFile << "----------------------------------" << endl;
	}
};
...
public:
	...
	void writeTrainToFile() {
		ifstream inFile("train.txt");
		string oldContent((istreambuf_iterator<char>(inFile)), istreambuf_iterator<char>());
		inFile.close();

		ofstream outFile("train_temp.txt");
		outFile << "New Train Data:" << endl;
		for (int i = 0; i < bog; i++) {
			if (bogies[i] != NULL) {
				bogies[i]->writeToFile(outFile);
			}
		}
		outFile << "----------------------------------" << endl;
		...
		remove("train.txt");
		rename("train_temp.txt", "train.txt");

		cout << "Train data prepended to train.txt successfully." << endl;
	}
};
